...
let result = new Set();
let block = document.getElementById("series1-div");
...
export function filterSeries() {  
  ...
  for (let i = 0; i < series.length; i++) {
    if(searchYears.size && searchGenres.size && searchCountries.size){
      ...
      if(searchYears.has(series[i].year) && searchGenres.has(series[i].genre))
      result.add(series[i]);
    }else if(searchYears.size && searchCountries.size){
      if(searchYears.has(series[i].year) && searchCountries.has(series[i].country))
      result.add(series[i]);
    }else if(searchGenres.size && searchCountries.size){
      if(searchGenres.has(series[i].genre) && searchCountries.has(series[i].country))
      result.add(series[i]);
    }else if(searchYears.size){
      if(searchYears.has(series[i].year))
      result.add(series[i]);
    }else if(searchGenres.size){
      if(searchGenres.has(series[i].genre))
      result.add(series[i]);
    }else if(searchCountries.size){
      if(searchCountries.has(series[i].country))
      result.add(series[i]);
    }else{
      return;
    }
